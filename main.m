...
B1 = 1000;
B2 = 1000;
quan = [0.05,0.95,0.025,0.975].*100;
true0 = [mu0,sig02,a,b,sigb2,re];
true02 = repmat(true0,1,2);
CP = zeros(size(true02,2)+prul*2,2);Length=CP;
...
for i = 1:B1
    ...
    for row = 1:size(sort_all,1)
        ...
        if  (sort_all(row,3)<=true(row))  &&  (true(row)<=sort_all(row,4))
            CP(row,2)=CP(row,2)+1;
        end
        Length(row,2)=Length(row,2)+sort_all(row,4)-sort_all(row,3);        
